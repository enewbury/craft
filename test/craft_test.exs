defmodule CraftTest do
  ...
  describe "query/3" do
    nexus_test "exits early when timeout is hit", %{name: name, nexus: nexus} do
      wait_until(nexus, {Stability, :all})
    ...
    nexus_test "concurrency setting affects blocking", %{name: name, nexus: nexus} do
      wait_until(nexus, {Stability, :all})

      Craft.query({:notify, self()}, name)
      assert_receive {:query_from, foreground_pid}

      Craft.query({:notify, self()}, name, concurrency: :non_blocking)
      assert_receive {:query_from, background_pid}

      assert foreground_pid != background_pid
