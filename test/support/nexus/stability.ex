...
defmodule Craft.Nexus.Stability do
  ...
  def handle_event({:sent_msg, follower, leader, %AppendEntries{} = append_entries}, %State{leader: leader} = state) do
  ...
  def handle_event({:sent_msg, _follower, leader, %AppendEntries{}}, state) do
    state = %{state | leader: leader} |> reset_counts()

    {:cont, state}
  end

  def handle_event({:sent_msg, leader, follower, %AppendEntries.Results{} = append_entries_results}, %State{leader: leader} = state) do
    counts =
